...
description: Research and rely on official first-party documentation before writing setup code, commands, technical reports, project plans, proposals, or presentation materials that depend on exact product, SDK, CLI, service, model, standard, policy, program, or vendor documentation. Use when an AI assistant or agent needs to produce installation/configuration/launch steps, shell commands, Docker files, startup scripts, inference commands, environment variables, troubleshooting guidance, or document/deck content where official terminology, requirements, capabilities, limitations, versions, dates, or source citations matter.
---

# Official Docs Setup

Use the official documentation as the source of truth before producing installation/runtime code or documentation-heavy deliverables such as technical reports, project plans, proposals, and presentation materials. Prefer current, first-party sources over memory, unofficial blog posts, generic package examples, or unsourced summaries.
...
1. Identify the exact target and deliverable before writing.

- Confirm the exact product, model, SDK, CLI, framework, or server the user means.
- For reports, plans, proposals, or presentations, confirm the relevant service, program, standard, policy, vendor product, model, method, audience, and output format.
- Infer relevant execution context from the repo or request when producing code: operating system, shell, language runtime, package manager, accelerator, container runtime, and version constraints.
- Ask a clarifying question only when multiple materially different setup paths, source authorities, or deliverable assumptions remain plausible.
...
- For reports, plans, and presentations, prefer official standards, specifications, policy pages, program calls, RFPs, institutional templates, release notes, data sheets, technical white papers, and maintainer-authored docs.
...
- For reports, plans, proposals, and presentations, capture exact official terminology, definitions, eligibility or compliance requirements, stated capabilities and limitations, version/date scope, deadlines, metrics, workflow steps, cited figures/tables, and required document sections.
- Note platform-specific branches when the docs split by OS, shell, package manager, CUDA/ROCm, or CPU/GPU support.
- Do not rely on remembered commands, product claims, policy details, standards requirements, or program dates for version-sensitive or source-sensitive content.

4. Write code, commands, or document content that mirrors the docs.
...
- For reports, plans, and decks, separate source-backed facts from interpretation, recommendations, or proposed work.
- Use the official terminology and constraints in the content. Do not turn aspirational or preview features into guaranteed capabilities.
- Preserve source traceability in citations, speaker notes, footnotes, appendix references, or a short source list, depending on the deliverable format.

5. Cite sources and state assumptions.

- Link the official sources used in the final answer.
- State important assumptions separately, such as OS, shell, Python version, CUDA version, container runtime, product version, document audience, deadline, or policy/date scope.
...
4. Official standards, government, institutional, or program-owner documentation when the task concerns compliance, reports, proposals, plans, or presentations rather than software setup.
5. Community content only to fill gaps left by official sources, and clearly label it as a fallback.
...
- `<standard or policy> official specification`
- `<program or grant> official guidelines`
- `<vendor> <product> official white paper`
- `<product> official limitations`
...
- Official terminology, definitions, and constraints
- Source-backed claims separated from recommendations or proposed work
- Citation placement suitable for the deliverable, such as slide notes, report references, or plan appendix

## Guardrails

- Do not invent package names, CLI flags, ports, model IDs, or environment variables.
- Do not invent official requirements, capabilities, compliance claims, deadlines, eligibility rules, or version support.
- Do not silently mix commands from different operating systems or package managers.
- Do not merge facts from different product versions, policy years, standards editions, or program calls without stating the scope.
- Do not omit vendor-documented prerequisites that affect whether the setup works.
- Do not present outdated or memory-based commands, policy details, or product claims as verified facts.
...
- For document and presentation tasks, inspect any user-provided source files, template, audience notes, or assignment requirements first, then verify external claims against official docs.
...
The skill is complete only when generated code, commands, or source-sensitive document claims can be traced back to official documentation, the assumptions and scope are visible, and the user has a concrete way to verify the installation/runtime result or audit the report/plan/presentation sources.
