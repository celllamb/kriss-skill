...
SECRET_PATTERNS = (
    ...
    re.compile(
        r"(?i)(\\?[\"']?\b(?:api[_-]?key|access[_-]?token|authorization|password|passwd|secret|private[_-]?key)\b\\?[\"']?\s*[:=]\s*\\?[\"']?)([^\\\"'\s,;}]+)"
    ),
    re.compile(
        r"(?i)(\b(?:authorization|proxy-authorization)\b\s*[:=]\s*(?:(?:bearer|basic|token)\s+)?)([^\s,;]+)"
    ),
    re.compile(
        r"(?i)(\b(?:api[_-]?key|access[_-]?token|auth(?:orization)?|password|passwd|secret|private[_-]?key)\b\s*[:=]\s*)([^\s,;]+)"
    ),
...
def changed_paths(repo_root: Path) -> List[str]:
    ...
    return sorted(path for path in paths if path and not path.replace("\\", "/").startswith(".review/"))
...
def ensure_no_active_clean_filters(repo_root: Path, paths: Sequence[str]) -> None:
    ...
    for relative_path in paths:
        try:
            result = subprocess.run(
                ["git", "check-attr", "filter", "--", relative_path.replace("\\", "/")],
                cwd=str(repo_root),
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                check=False,
...
def changed_worktree_paths(repo_root: Path) -> List[str]:
    ...
    return sorted(
        ...
        for path in paths
        if path
        and path.replace("\\", "/") != ".review"
        and not path.replace("\\", "/").startswith(".review/")
    )
...
def literal_pathspec(relative_path: str) -> str:
    return ":(literal)" + relative_path.replace("\\", "/")
...
def format_untracked(entries: Sequence[UntrackedEntry]) -> str:
    if not entries:
        return "(없음)"
    blocks: List[str] = []
    for relative_path, size, content, digest in entries:
        header = f"--- {prompt_data(relative_path)} ({size} bytes) ---"
        if digest is not None or content is None or b"\0" in content[:8192]:
            content_digest = digest or hashlib.sha256(content or b"").hexdigest()
            blocks.append(
                header
                + "\n[binary or large file; inspect it with Read if needed; sha256="
                + content_digest
                + "]"
            )
            continue
...
def build_review_prompt(
    ...
    user_request = request.get("user_request") or request.get("prompt") or request.get("objective") or "(미기재)"
    request_json = prompt_data(json.dumps(request, ensure_ascii=False, indent=2))
    acceptance_criteria = prompt_data(
        json.dumps(request.get("acceptance_criteria", []), ensure_ascii=False, indent=2)
    )
    ...
    previous_review = prompt_data(
        json.dumps(request.get("previous_review", {}), ensure_ascii=False, indent=2)
    )
...
{prompt_data(str(user_request))}
</untrusted-user-request>

<untrusted-codex-request-json>
{request_json}
</untrusted-codex-request-json>
...
def _run_once(argv: Optional[Sequence[str]] = None) -> int:
    ...
    prompt = build_review_prompt(request, fingerprint, paths, agents, unstaged, staged, untracked)
    started_at = utc_now()
    state: Dict[str, Any] = {
    ...
    try:
        ...
        command = build_command(binary, settings, options, prompt=None if options.get("input_format") else prompt)
    except FileNotFoundError:
        message = "Claude Code CLI를 실행할 수 없습니다."
