...
4. From the repository root, run exactly one Claude review round:

   ```text
   python3 .agents/skills/claude-review-loop/scripts/run_review.py
   ```

   On Windows PowerShell, `python` is also acceptable when `python3` is not registered. Always read both `.review/response.json` and `.review/state.json` after the command. The script performs one Claude call only; it never loops until approval.
...
For an explicit post-approval comparison without contacting Claude, run `python .agents/skills/claude-review-loop/scripts/run_review.py --print-fingerprint` and compare its output with `response.json.reviewed_fingerprint`.
